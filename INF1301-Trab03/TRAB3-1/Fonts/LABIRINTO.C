...
LBT_tpCondRet LBT_SolucionarLabirinto ( LBT_tppLabirinto pLabirinto, int * steps )
{
	...
	/* Salva a identidade do vértice corrente */
	
	idVerticeRetorno = pLabirinto->idVerticeCorrente;
	
	/* Obtém o afastamento horizontal e vertical entre a entrada e a saída do labirinto para constituir uma heurística */
	...
	/* Ir para a entrada do labirinto a partir da qual se procurará a posição de saída do labirinto */
	...
	/* Cria lista para armazenar os sentidos em ordem certa para ir da posição de entrada à posição de saída do labirinto */
	...
	/* Loop que avalia cada situação de decisão para percorrer um caminho com o objetivo de encontrar a saída */
	...
	while( status == 0 )
	{
		...
		/* Caso tenha encontrado a saída do labirinto */
		...
		if( afastamentoHorizontal == 0 && afastamentoVertical == 0 && LBT_CondRetObtido == LBT_CondRetSaidaEncontrada && retorno == 0 )
		{
			...
			/* Percorre o labirinto seguindo os sentidos da lista de solução e criando uma lista com os vértices do caminho da solução */
		...
		/* Decide percorrer norte ou sul de acordo com o afastamento vertical heurístico */
		...
		/* Decide percorrer oeste ou este de acordo com o afastamento horizontal heurístico caso não tenha ocorrido movimento vertical */
		...
		/* Decide percorrer para vértices ainda não visitados caso não tenha sido possível percorrer nos sentidos prioritários da heurística */
		...
		/* Caso não haja vértices não visitados, decide percorrer para um vértice já visitado que corresponda ao oposto do último sentido armazenado na lista de solução */
		/* Caso consiga, elimina o último elemento desta lista de solução */
		...
		if( retorno == 1 )
		{
			...
			/* Se não foi posível percorrer no sentido anterior oposto ao último da lista de solução, decide percorrer para um vértice já visitado */
		...
		/* Se não foi possível percorrer em um sentido até aqui no labirinto, então percorre-se no labirinto no último sentido percorrido no ciclo anterior deste while */
		/* Não pode haver percurso no último sentido salvo se se trata do primeiro movimento do solucionador. Nesse caso, labirinto sem solução. */
...
/***********************************************************************
*
*  $FC Função: LBT -Destruir conteúdo do labirinto
*
*  $ED Descrição da função
*     Destroi o conteúdo do labirinto (conteúdo do labirinto).
*
***********************************************************************/
...
} /* Fim função: LBT -Destruir conteúdo do labirinto */
